...
def points_updated(
    ...
    for points_type in ["positive", "negative"]:
        updated_points[points_type] = widget.get_updated_points(
            state=state, data=None, points_type=points_type
        )
        removed_points[points_type] = widget.get_removed_points(
            state=state, data=None, points_type=points_type
        )
    ...
    if widget.is_active:
        ...
        for points_type in ["positive", "negative"]:
            ...
            for removed_point in removed_points_by_type:
                local_functions.remove_point_from_connected_cards(
        ...
        for points_type in ["positive", "negative"]:
            ...
            for updated_point in updated_points_by_type:
                updated_point["relative"] = widget.get_relative_coordinates(
                    updated_point
                )
                local_functions.add_point_to_active_cards(
                    origin_identifier=identifier,
...
def clean_up(
    ...
    for widget in g.grid_controller.widgets.values():
        if (
            widget.is_active
            and not widget.is_empty
            and not widget.is_broken
            and not widget.is_finished
        ):
            widget.clean_up()
...
def assign_base_points(
    ...
    for widget in g.grid_controller.widgets.values():
        if (
            ...
            w_padding, h_padding = (state["bboxesPadding"] / 100) * w / 4, (
                state["bboxesPadding"] / 100
            ) * h / 4

            x0, y0, x1, y1 = (
                widget.scaled_bbox[0][0] + int(w_padding),
            ...
            widget.negative_points.append(
                {"position": [[x0, y0]], "id": f"{uuid.uuid4()}"}
            )
            widget.negative_points.append(
                {"position": [[x0, y1]], "id": f"{uuid.uuid4()}"}
            )
            widget.negative_points.append(
                {"position": [[x1, y0]], "id": f"{uuid.uuid4()}"}
            )
            widget.negative_points.append(
                {"position": [[x1, y1]], "id": f"{uuid.uuid4()}"}
            )

            # new negative points between corners
            widget.negative_points.append(
                {"position": [[x0, (y0 + y1) // 2]], "id": f"{uuid.uuid4()}"}
            )
            widget.negative_points.append(
                {"position": [[x1, (y0 + y1) // 2]], "id": f"{uuid.uuid4()}"}
            )
            widget.negative_points.append(
                {"position": [[(x0 + x1) // 2, y0]], "id": f"{uuid.uuid4()}"}
            )
            widget.negative_points.append(
                {"position": [[(x0 + x1) // 2, y1]], "id": f"{uuid.uuid4()}"}
            )

            # new positive point in center
            center_x, center_y = int((x0 + x1) / 2), int((y0 + y1) / 2)
            widget.positive_points.append(
                {"position": [[center_x, center_y]], "id": f"{uuid.uuid4()}"}
            )
...
def update_masks(
    ...
    try:
        response = g.api.task.send_request(
            int(state["processingServer"]["sessionId"]),
            "smart_segmentation_batched",
            data={},
            context={"data_to_process": data_to_process},
            timeout=60,
        )
        local_functions.update_local_masks(response, state)
    except Exception as ex:
        dialog_window.notification_box.title = (
            "The model is not responding to requests."
        )
        dialog_window.notification_box.description = (
            "Please make sure the model is working and reconnect to it."
        )
...
def next_batch(
    ...
    try:
        ...
        for widget in g.grid_controller.widgets.values():
            ...
            widgets_data_by_datasets.setdefault(widget.dataset_name, []).append(
                widget_data
            )

        # 2 - load new data to widgets
        g.grid_controller.clean_all(state=state, data=DataJson())
        g.grid_controller.change_count(
            actual_count=state["windowsCount"],
            app=g.app,
            state=state,
            data=DataJson(),
        ...
        for current_dataset_name, widget_data in widgets_data_by_datasets.items():
            if isinstance(current_dataset_name, str):
                ds_id = f.get_dataset_id_by_name(
                    current_dataset_name, state["outputProject"]["id"]
                )
        ...
        supervisely.logger.warning(repr(e))
        logger.critical(
            "Unexpected exception in main.",
            exc_info=True,
            extra={
                "main_name": "next_batch",
                "exc_str": repr(e),
            },
        )
        raise e
...
def bbox_updated(
    ...
    scaled_width, scaled_height = updated_widget.get_bbox_size(
        updated_widget.scaled_bbox
    )
    ...
    run_sync(DataJson().synchronize_changes())
    background_tasks.add_task(
        local_functions.update_single_widget_realtime,
        widget=updated_widget,
        state=state,
    )


def bboxes_masks_opacity_changed(
    state: supervisely.app.StateJson = Depends(supervisely.app.StateJson.from_request),
):
    g.grid_controller.update_local_fields(state=state, data=DataJson())
    for widget in g.grid_controller.widgets.values():
